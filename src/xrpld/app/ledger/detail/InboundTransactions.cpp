...
public:
        ...
        std::shared_ptr<protocol::TMLedgerData> packet_ptr) override
    {
        ...
        if (ta == nullptr)
        {
            peer->charge(Resource::feeUnwantedData, "ledger_data");
            return;
        }
        ...
        for (auto const& node : packet.nodes())
        {
            ...
            {
                peer->charge(Resource::feeInvalidRequest, "ledger_data");
                return;
            }
            ...
            if (!id)
            {
                peer->charge(Resource::feeBadData, "ledger_data");
                return;
            }
        ...
        if (!ta->takeNodes(data, peer).isUseful())
            peer->charge(Resource::feeUnwantedData, "ledger_data not useful");
    }
