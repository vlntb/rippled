    ...
    protocol::TMLedgerData& packet)
{
    if (packet.type() == protocol::liBASE)
    {
        if (packet.nodes().empty())
        {
            ...
            peer->charge(
                Resource::feeInvalidRequest, "ledger_data empty header");
            return -1;
        }
        ...
        catch (std::exception const& ex)
        {
            ...
            peer->charge(Resource::feeBadData, "ledger_data "s + ex.what());
            return -1;
        }
        ...
        (packet.type() == protocol::liAS_NODE))
    {
        ...
        if (packet.nodes().empty())
        {
            ...
            peer->charge(Resource::feeInvalidRequest, "ledger_data no nodes");
            return -1;
        }
        ...
        for (auto const& node : packet.nodes())
        {
            if (!node.has_nodeid() || !node.has_nodedata())
            {
                ...
                peer->charge(
                    Resource::feeInvalidRequest, "ledger_data bad node");
                return -1;
            }
        }
